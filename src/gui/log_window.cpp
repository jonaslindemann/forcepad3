...
std::shared_ptr<LogWindow> gui::LogWindow::create(const std::string name)
{
    return std::make_shared<LogWindow>(name);
}

void gui::LogWindow::clear()
{
    m_buffer.clear();
    m_lineOffsets.clear();
...
void gui::LogWindow::log(const std::string message)
{
